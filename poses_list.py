...
def generate(poses_detail, unique_no):
    contents = [
        types.Content(
            role="user",
            parts=[
                types.Part.from_text(text=f"""Could you please generate realistic human male or women model image based on a pose: {poses_detail}"""
                ),
            ],
        )
    ]
    ...
    for chunk in client.models.generate_content_stream(
        ...
        if chunk.candidates[0].content.parts[0].inline_data and chunk.candidates[0].content.parts[0].inline_data.data:
            file_name = f"pose_new_{unique_no}.png"
            file_index += 1
            inline_data = chunk.candidates[0].content.parts[0].inline_data
            data_buffer = inline_data.data
            file_extension = mimetypes.guess_extension(inline_data.mime_type)
            save_binary_file(f"model_poses/{file_name}{file_extension}", data_buffer)
        else:
            print(chunk.text)

for num,poses_detail in enumerate(poses):
    generate(poses_detail, num)

[182, 84, 184, 185, 186, 187]
