...
CRITICAL REQUIREMENTS:
1. FACE: Use the EXACT face from the reference face image - preserve all facial features, expressions, skin tone, and characteristics with 100% accuracy
2. LOWER GARMENT: Replicate the EXACT lower garment from reference image - match fabric texture, color saturation, pattern details, fit precision, style elements, waistband details, and garment construction identically
3. UPPER GARMENT: Wear complementary {upper_garment_type} that harmonizes with the lower garment - coordinate colors, maintain style consistency, ensure appropriate fit and silhouette
4. POSE: {body_pose} with natural, confident posture
5. BACKGROUND: {background_description}
